...
def FormatModuleExamInfo(module_name, date, time, venue):
    try:
    ...
    info = f"{module_name} exam is at {time} on {date_formatted} at {venue}"
    UpdateModule(module_name, info)
    print(f"Module '{module_name}' updated with info: {info}")
